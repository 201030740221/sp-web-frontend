# 自动补全
define ['./tpl-layout.hbs',
        './tpl-item.hbs'], (layout_template, item_template)->
    class AutoComplete
        constructor: (@options) ->
            #this.options = {}
        ...
        listLoad: ->
            ...
            if( !self.autocomplete_element or !self.autocomplete_element.length)
                ...
                self.autocomplete_element.css
                    display: displayVal
                    width: (self.options.width || self.options.$target.outerWidth())
                    top: self.options.$target.offset().top + self.options.$target.height()
                    left: self.options.$target.offset().left
                # 关闭
