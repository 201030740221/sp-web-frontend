...
define ()->
    GoodTypeSelect = (options)->
        ...
        setPrice = (key,jump, isColor=false)->
        ...
        setState = (jump=false,isColor=null)->
            #已经选择的节点
            selectedObjs = $('.sku-attr._active')
            ...
            $('.sku-attr').not(selectedObjs).not(self).not('[data-main="true"]').each ->
                ...
                testAttrIds.sort (value1, value2) ->
                    parseInt(value1) - parseInt(value2)
                #console.log(SKUResult[testAttrIds.join(';')],testAttrIds.join(';'))
